...
export default function DateRangePicker({
    ...
    startDate,
    endDate,
    maxDate, // 1. Added maxDate prop here
    onDateChange,
    error,
    disabled = false,
    required = false
    ...
    // Initialize simply to the current date
    const [viewDate, setViewDate] = useState(new Date());

    const errorMessage = typeof error === 'object' && error !== null ? error.message : error;

    // Add a useEffect to reset the view whenever the calendar opens
    useEffect(() => {
        if (isOpen) {
            if (startDate) {
    ...
    const renderDayCell = (day, type, baseYear, baseMonth) => {
        ...
        // 2. Determine if the cell is past the maxDate (YYYY-MM-DD string comparison works perfectly)
        const isFuture = maxDate && cellDateStr > maxDate;

        // 3. Disable if it's off-month or in the future
        const isDisabledCell = isOffMonth || isFuture;

        return (
            <div
                key={`${type}-${cellDateStr}`}
                className={`w-full flex items-center justify-center ${inRange && !isFuture ? 'bg-[#eaf4fd]' : ''}`}
            >
                <button
                    type="button"
                    onClick={(e) => {
                    ...
                    className={`w-8 h-8 flex items-center justify-center rounded text-sm transition-colors
                        ...
                        ${!isStart && !isEnd && !isDisabledCell ? 'text-gray-700 hover:bg-gray-100 cursor-pointer' : ''}
                        ${isOffMonth && !inRange && !isFuture ? 'text-gray-300 cursor-default' : ''}
                        ${isOffMonth && inRange && !isFuture ? 'text-gray-400 cursor-default' : ''}
                        ${isFuture ? 'text-gray-200 bg-gray-50 cursor-not-allowed' : ''} /* Future styling */
                    `}
                >
                    {day}
                </button>

...
import { get_ticket_by_serial_number_service, get_product_registration_by_serial_number_service } from "../services/tickets-service";
...
export function get_product_registration_by_serial_number_thunk(ticket_id) {
    return async function (dispatch, getState) {
        const result = await get_product_registration_by_serial_number_service(ticket_id);
        dispatch(appSlice.actions.setTicket(result.data));
    };
}
